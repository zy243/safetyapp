// controllers/followMeController.js (ESM version)
...
export const startFollowMe = async (req, res) => {
    try {
        ...
        let followMe = await FollowMe.findOne({ user: req.user.id, isActive: true });
        if (followMe) {
            return res.status(400).json({ success: false, message: 'Follow Me session already active' });
        }
        ...
        if (shareWith && Array.isArray(shareWith)) {
            for (const contactId of shareWith) {
                ...
                if (contact && user.trustedCircle.some(c => c.phone === contact.phone)) {
                    sharingList.push({ userId: contactId, expiresAt });
                }
            }
        }
        ...
        user.followMe.lastLocation = { latitude, longitude, timestamp: new Date() };
        await user.save();
...
export const updateLocation = async (req, res) => {
    try {
        ...
        if (!followMe) {
            return res.status(404).json({ success: false, message: 'No active Follow Me session' });
        }
        ...
        if (new Date() > followMe.expiresAt) {
            ...
            return res.status(400).json({ success: false, message: 'Follow Me session has expired' });
        }
        ...
        res.status(200).json({ success: true, location: locationUpdate });
    } catch (err) {
        res.status(500).json({ success: false, message: err.message });
    }
};
...
export const stopFollowMe = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: 'Follow Me session stopped' });
    } catch (err) {
        res.status(500).json({ success: false, message: err.message });
    }
};
...
export const getFollowMeStatus = async (req, res) => {
    try {
        const followMe = await FollowMe.findOne({ user: req.user.id, isActive: true });
        if (!followMe) return res.status(200).json({ success: true, isActive: false });
...
export const getSharedLocation = async (req, res) => {
    try {
        ...
        if (!followMe) return res.status(404).json({ success: false, message: 'Location not shared with you' });
    ...
    try {
        ...
        res.status(200).json({
            ...
            sessions: followMeSessions.map(session => ({
                ...
                duration: Math.round((session.expiresAt - session.startedAt) / 1000 / 60),
...
export const updateSettings = async (req, res) => {
    try {
        ...
        if (!followMe) return res.status(404).json({ success: false, message: 'No active Follow Me session' });
        ...
        await followMe.save();
        res.status(200).json({ success: true, settings: followMe.settings });
    } catch (err) {
        res.status(500).json({ success: false, message: err.message });
    }
};

// Helper: notify trusted contacts
